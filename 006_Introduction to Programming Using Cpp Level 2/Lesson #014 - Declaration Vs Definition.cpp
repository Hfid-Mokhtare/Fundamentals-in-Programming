...
//i practice the lesson in my previouse progect "MathGame" i add to it declarations 
...
int SimpleCalculator(int, int, enOperationType);
void StartGame();
void ResetScreen();
void PrintQuizzResult(stQuizz);
void AskAndCorrectQuestionListAnswers(stQuizz&);//
void PlayMathGame();
string GetFinalResultsText(bool);
string GetOperationTypeText(enOperationType);
string GetQuestionLevelText(enQuestionLevel);
short GetRandomNumber(int, int);
void SetScreenColor(bool);
int ReadHowManyQuestions();
enQuestionLevel ReadQuestionLevel();
enOperationType ReadOperationType();
short GetRandomOperationType();
stQuestion GenerateQuestion(enQuestionLevel, enOperationType);
void GenerateQuizzQuestions(stQuizz&);//
int ReadQuestionAnswer();
void PrintTheQuestion(stQuizz&, short);//
void CorrectTheQuestionAnswer(stQuizz&, short);//
...
enQuestionLevel ReadQuestionLevel()
{
	...
	do
	{
		cout << "\nEnter QuestionLevel : [1] Easy, [2] Med, [3] Hard, [4] Mix ? ";
		cin >> Level;
